...
from connectors.web.bilibili import fetch_bilibili_dynamic, fetch_bilibili_dynamic_with_page
...
def fetch_web_many(sources: list[dict], limit: int = 12, timeout_ms: int = 60000) -> list[FeedFetchResult]:
    ...
    try:
        with sync_playwright() as playwright:
            web_browser = playwright.chromium.launch(headless=True)
            web_page = web_browser.new_page(user_agent=USER_AGENT)
            weibo_context = None
            weibo_page = None
            x_context = None
            x_page = None
            for source in sources:
                ...
                if target.site == "bilibili":
                    results.append(fetch_bilibili_dynamic_with_page(web_page, source, limit=limit, timeout_ms=timeout_ms))
                    continue
