        ...
        rows.append(
            {
                ...
                "enabled_text": "??" if item.get("enabled", True) else "??",
...
def render_sources_management(params: dict[str, str], sort_key: str, direction: str) -> str:
    ...
    for item in rows:
        ...
        toggle_text = "停用" if item["enabled"] else "生效"
        toggle_confirm = "确认停用该订阅源？" if item["enabled"] else "确认启用该订阅源？"
        ...
        body.append(
            ...
            "<td>"
            f"<a href='/source-toggle?id={html.escape(item['id'])}&enabled={'0' if item['enabled'] else '1'}' onclick=\"return confirm('{html.escape(toggle_confirm)}')\">{toggle_text}</a> / "
            f"<a href='{html.escape(edit_href)}'>编辑</a> / "
            f"<a href='/source-delete?id={html.escape(item['id'])}' onclick=\"return confirm('确认删除该订阅源？')\">删除</a>"
            "</td>"
            "</tr>"
        )
    ...
    if editing:
        modal = (
            ...
            "<div class='panel-head'><h3>编辑订阅源</h3><a class='modal-close' href='/?view=settings'>关闭</a></div>"
            "<form method='post' action='/source-save' class='form-grid'>"
            f"<input type='hidden' name='source_id' value='{html.escape(editing['id'])}'>"
            f"<label>名称<input name='name' value='{html.escape(editing.get('name', ''))}' required></label>"
            f"<label>站点 URL<input name='site_url' value='{html.escape(editing.get('site_url', ''))}'></label>"
            f"<label>RSS URL<input name='feed_url' value='{html.escape(editing.get('feed_url', ''))}' required></label>"
            "<div class='modal-actions'>"
            "<button class='btn' type='submit'>保存</button>"
            "<a class='btn ghost' href='/?view=settings'>取消</a>"
            "</div>"
            "</form>"
            "</div>"
            "</div>"
    ...
    return (
        ...
        f"<input type='hidden' name='sort' value='{html.escape(sort_key)}'>"
        f"<input type='hidden' name='dir' value='{html.escape(direction)}'>"
        f"<input class='search' name='source_q' value='{html.escape(query)}' placeholder='搜索名称 / RSS URL'>"
        "<button class='btn' type='submit'>搜索</button>"
        "</form>"
        "</div>"
        "<table><thead><tr>"
        f"<th>{text_sort_button('settings', '名称', 'name', sort_key, direction, sort_params)}</th>"
        f"<th>{text_sort_button('settings', '生效', 'enabled_sort', sort_key, direction, sort_params)}</th>"
        f"<th>{text_sort_button('settings', '来源类型', 'provider_label', sort_key, direction, sort_params)}</th>"
        f"<th>{time_sort_button('settings', '条目数', 'entry_count', sort_key, direction, sort_params)}</th>"
        f"<th>{time_sort_button('settings', '失效天数', 'invalid_sort', sort_key, direction, sort_params)}</th>"
        "<th>操作</th>"
        "</tr></thead>"
        f"<tbody>{''.join(body)}</tbody></table>"
        "</section>"
...
def render_settings_view(params: dict[str, str], sort_key: str, direction: str) -> str:
    ...
    return (
        ...
        "<section class='card'>"
        "<h2>运行状态</h2>"
        f"<p>服务地址：http://{HOST}:{PORT}</p>"
        f"<p>最近执行：{html.escape(format_datetime(status.get('last_run_at', '')))}</p>"
        f"<p>最近成功：{html.escape(format_datetime(status.get('last_success_at', '')))}</p>"
        f"<p>抓取结果：{status.get('last_total_sources', 0)} / 成功 {status.get('last_success_sources', 0)}</p>"
        f"<p>新增条目：{status.get('last_inserted_entries', 0)}</p>"
        "</section>"
        "<section class='card fetch-card'>"
        "<h2>手动抓取</h2>"
        "<p>系统会在 06:00 和 16:00 自动抓取一次，也可以在这里立即执行。</p>"
        "<div class='actions'>"
        "<a class='btn' href='/fetch-now'>立即抓取</a>"
        "</div>"
        "</section>"
        "<section class='card'>"
        "<details>"
        ...
        "<section class='card'>"
        "<h2>Laterhub 概览</h2>"
        f"<p>总数：{summary.get('total_count', 0)}</p>"
        f"<p>未完成：{summary.get('unfinished_count', 0)}</p>"
        f"<p>已完成：{summary.get('finished_count', 0)}</p>"
        "</section>"
        "<section class='card'>"
        "<h2>系统说明</h2>"
        "<p>当前仓库由多个小模块组成，分别处理订阅抓取、收藏同步和后续整理。</p>"
        "<p>Subscriptions 负责管理 RSS / 网页源 / 抓取结果，订阅数据落在 `rss.sqlite3`。</p>"
        "<p>Laterhub 负责沉淀需要稍后处理的内容，来源包括 API / 浏览器收藏 / 人工补录，数据落在 `info_hub.db`。</p>"
        "<p>Newshub 预留给后续资讯聚合扩展，目前不参与主流程。</p>"
        "<p>网页登录态抓取依赖本地浏览器 profile，切换环境后需要确认对应 profile 仍可用。</p>"
        "</section>"
        "<section class='card'>"
        "<h2>Laterhub 来源</h2>"
        "<table><thead><tr>"
        "<th>来源</th><th>用途说明</th><th>抓取方式</th><th>总数</th><th>未完成</th>"
        "</tr></thead>"
